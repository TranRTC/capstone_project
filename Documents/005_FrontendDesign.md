...
- **State:** Component state + `localStorage` for auth (no global Context store)
- **HTTP Client:** Axios (`src/services/api.ts`)
- **Real-Time:** `@microsoft/signalr` v8 (`src/services/signalRService.ts`)
- **Charts:** Recharts (`DeviceTemperatureChart`, `RealTimeChart`)
- **UI:** Material-UI (MUI) v5 + Emotion
- **Config:** `REACT_APP_API_BASE_URL`, `REACT_APP_SIGNALR_HUB_URL` (defaults: port **5000**)
...
│   └── components/
│       ├── common/     Navigation, ProtectedRoute, ConfirmDialog
│       ├── charts/     DeviceTemperatureChart, RealTimeChart
│       ├── device/     DeviceForm
│       ├── sensor/     SensorForm
│       └── actuator/   ActuatorForm
├── package.json
└── .env.example
```

### 2.3 Routes (as implemented in `App.tsx`)

| Path | Page | Access |
|------|------|--------|
| `/login` | Login | Public |
| `/dashboard` | Dashboard | Authenticated |
| `/devices` | Devices list | Authenticated |
| `/devices/:id` | Device detail (sensors, actuators, rules, chart, commands) | Authenticated |
| `/sensors` | Sensors (all devices) | Authenticated |
| `/actuators` | Actuators | Authenticated |
| `/alert-rules` | Alert rules | Authenticated |
| `/alerts` | Alerts | Authenticated |
| `/command-history` | Command history | Authenticated |
| `/users` | User management | Admin only |

Write access (create/edit/delete): **Admin** and **Operator** via `authService.isOperatorOrAbove()`.
...
const API_BASE_URL = process.env.REACT_APP_API_BASE_URL || 'https://localhost:5000/api/v1';
