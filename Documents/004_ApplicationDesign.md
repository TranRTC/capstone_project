...
- MQTT ingest and device command dispatch
...
- **Real-Time:** SignalR (`MonitoringHub` at `/monitoringhub`)
- **MQTT:** MQTTnet hosted service (subscribe readings, publish commands)
- **Auth:** JWT Bearer (BCrypt passwords, roles Admin/Operator/Viewer)
- **Database:** SQL Server
- **Language:** C#
...
│   ├── Controllers/       # REST API (route prefix api/v1/...)
│   ├── Hubs/MonitoringHub.cs
│   ├── Services/          # MqttService, MqttCommandDispatcher, SignalRNotificationService
│   └── Program.cs         # DI, JWT, CORS, Swagger (dev), migrations, admin seed
├── IoTMonitoringSystem.Core/
│   ├── Entities/          # 11 entities
│   ├── DTOs/
│   └── Interfaces/        # IAuthService, INotificationService
├── IoTMonitoringSystem.Infrastructure/
│   ├── Data/ApplicationDbContext.cs
│   ├── Repositories/
│   └── Migrations/
└── IoTMonitoringSystem.Services/
    ├── DeviceService, SensorService, SensorReadingService
    ├── AlertService, AlertRuleService
    ├── ActuatorService, DeviceCommandService, DeviceConfigurationService
    └── AuthService
```

**Development base URL:** `http://localhost:5000` (see `appsettings.Development.json` → `Kestrel:Endpoints:Http:Url`).
...
All controllers except `HealthController` and `POST /auth/login` require `Authorization: Bearer <JWT>`.
Write operations (POST/PUT/DELETE) on devices, sensors, actuators, alert rules, alerts, and commands require role **Admin** or **Operator**.
...
### 5.5a Additional implemented controllers

| Area | Route prefix | Notes |
|------|----------------|-------|
| Auth | `api/v1/auth` | `login`, `users`, `change-password`, `users/{id}/role` |
| Health | `api/v1/health` | Anonymous; `GET`, `GET mqtt` |
| Actuators | `api/v1/actuators` | Nested under `devices/{deviceId}/actuators` |
| Device commands | `api/v1/devices/{deviceId}/commands` | POST command; GET history; `GET api/v1/commands/{commandId}` |
| Device configurations | `api/v1/devices/{deviceId}/configurations` | GET/PUT key-value settings |

### 5.5b MQTT (as implemented)

| Direction | Topic | Purpose |
|-----------|--------|---------|
| Subscribe | `devices/+/sensors/+/readings` | Ingest sensor readings |
| Subscribe | `sensor/reading/+/+` | Alternate reading format |
| Subscribe | `devices/+/commands/ack` | Command acknowledgements |
| Publish | `devices/{deviceId}/commands` | Send command payload to edge device |

Hosted services: `MqttService`, `CommandTimeoutService` (default timeout 5 minutes).
