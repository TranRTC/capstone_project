...
| M5.4a | Live analog line chart (SCADA scroll) | **Live** tab: while filling, line grows from the right; after ~one window of data, caption says **Scrolling** — full-width trace, new points right, oldest drops left (~1 Hz); not on 10m/1h/24h; close dialog → dashboard SignalR OK | ☐ |
