...
- HTTP: `http://localhost:5000` (configured in `appsettings.Development.json`)
- Swagger UI (development only): `http://localhost:5000/swagger`
- SignalR hub: `http://localhost:5000/monitoringhub`
...
### 6.1 Frontend project (as built)

This repository uses **Create React App** with TypeScript at `Frontend/iot-monitoring-frontend/`.
...
# Charts (installed in this project)
...
See `src/config/runtimeConfig.ts` for defaults when env vars are omitted.
...
VITE_SIGNALR_URL=http://localhost:5000/hubs/monitoring
