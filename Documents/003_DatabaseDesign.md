...
The database is implemented in SQL Server via **Entity Framework Core** (`ApplicationDbContext` in `Backend/IoTMonitoringSystem.Infrastructure/Data/`). Migrations are under `Infrastructure/Migrations/`. Presentation slides may include a visual ERD in `Documents/Presentation/`.
...
Devices (1) ────< (Many) DeviceCommands
Devices (1) ────< (Many) Actuators
Actuators (0..1) ─── FeedbackSensor (Sensor)
...
The database consists of **11 tables** (15 configured EF relationships):

1. **Devices** (3.1) — Core device/equipment information
2. **Sensors** (3.2) — Sensor definitions
3. **SensorReadings** (3.3) — Time-series sensor data (highest volume)
4. **DeviceStatusHistories** (3.4) — Device status change log
5. **OperationalMetrics** (3.5) — Calculated KPIs (schema present; limited API/UI use)
6. **AlertRules** (3.6) — Alert rule definitions
7. **Alerts** (3.7) — Triggered alert records
8. **DeviceConfigurations** (3.8) — Key/value device settings
9. **Actuators** (3.9) — Actuators linked to devices (optional feedback sensor)
10. **DeviceCommands** (3.10) — Commands sent to devices (MQTT), with status lifecycle
11. **Users** (3.11) — Application users (JWT authentication)

**Table relationships:**
- **Devices** is the central hub for sensors, readings, alerts, actuators, and commands
- **Users** is independent of device data
- **Alerts** reference **AlertRules**, **Devices**, and optionally **Sensors**
...
**Purpose:** Controllable outputs on a device (relays, motors, valves). Optional link to a feedback sensor.

| Column Name | Data Type | Description |
|------------|-----------|-------------|
| ActuatorId | INT | Primary key |
| DeviceId | INT | FK → Devices |
| Name, Description | NVARCHAR | Display metadata |
| Kind | NVARCHAR | `Discrete` or `Analog` |
| Channel | NVARCHAR | Hardware channel identifier |
| AnalogMin, AnalogMax, ControlUnit | DECIMAL/NVARCHAR | Analog control range |
| FeedbackSensorId | INT | Optional FK → Sensors |
| LastKnownState, LastStateAt | NVARCHAR/DATETIME2 | Last reported state |
| IsActive, CreatedAt, UpdatedAt | BIT/DATETIME2 | Standard fields |

---

### 3.10 DeviceCommands Table

**Purpose:** Commands sent from the UI/API to edge devices via MQTT, with lifecycle tracking.

| Column Name | Data Type | Description |
|------------|-----------|-------------|
| CommandId | BIGINT | Primary key |
| DeviceId | INT | FK → Devices |
| ActuatorId | INT | Optional FK → Actuators |
| CommandType | NVARCHAR | e.g. SetPower, SetValue |
| Payload | NVARCHAR | JSON command body |
| Status | NVARCHAR | Pending, Sent, Acked, Failed, Timeout |
| CorrelationId | NVARCHAR | Tracking id |
| RequestedBy | NVARCHAR | Username from JWT |
| CreatedAt, SentAt, CompletedAt | DATETIME2 | Timestamps |
| ErrorMessage | NVARCHAR | Failure detail |

---

### 3.11 Users Table

**Purpose:** Application users for JWT login and role-based UI/API access.

| Column Name | Data Type | Description |
|------------|-----------|-------------|
| UserId | INT | Primary key |
| Username | NVARCHAR | Unique login name |
| Email | NVARCHAR | User email |
| PasswordHash | NVARCHAR | BCrypt hash |
| Role | NVARCHAR | `Admin`, `Operator`, or `Viewer` |
| IsActive | BIT | Account enabled |
| CreatedAt, UpdatedAt | DATETIME2 | Audit fields |
...
- `DeviceStatusHistory` → DeviceStatusHistories
- `OperationalMetric` → OperationalMetrics
...
- **User Management:** Implemented in `Users` table (see 3.11)
