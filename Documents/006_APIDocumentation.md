...
- **Base URL:** Development `http://localhost:5000/api/v1` (Production: configure per deployment)
...
### 1.4 Authentication
**Authentication:** JWT Bearer tokens. Call `POST /api/v1/auth/login` with username/password, then send `Authorization: Bearer <token>` on protected endpoints. SignalR passes the token as query parameter `access_token` when connecting to `/monitoringhub`. Default development user (seeded): `admin` / `Admin@123`.
...
async function getDevices() {
  try {
    const response = await fetch('http://localhost:5000/api/v1/devices');
...
**Connection URL:** `http://localhost:5000/monitoringhub`
...
- JWT authentication and role-based authorization are implemented (Admin, Operator, Viewer). OAuth/SSO are future enhancements
