...
**Implementation note:** The system implements **JWT authentication** and **role-based access** with three roles: **Admin**, **Operator**, and **Viewer**. Admin manages users; Operator and Admin can create/update/delete devices, sensors, actuators, and alert rules; Viewer has read-only access. OAuth and enterprise SSO remain future enhancements.
...
- **Acceptance Criteria:**
  ...
  - Operator and Admin: create/update/delete devices, sensors, actuators, alert rules, commands
...
- **Status:** **Implemented**
- **Acceptance Criteria:**
  - JWT issued on `POST /api/v1/auth/login`
  - Bearer token required on protected API endpoints
  - SignalR accepts token via `access_token` query string
...
- Fine-grained per-resource permissions beyond Admin/Operator/Viewer
- Mobile application
